...
/* USER CODE BEGIN PTD */
typedef enum {
	ANIMAL_STATE_CALM, ANIMAL_STATE_EAT, ANIMAL_STATE_WALK
} AnimalState_t;
...
#define BATTERY_WARNING_INTERVAL_MS   180000UL
#define LED_BLINK_PERIOD_MS        200UL
#define LED_BLINK_TOGGLE_COUNT          6

//-----LoRaWAN
#define LORAWAN_SENDING_PERIOD    120000UL

//GNSS
#define GNSS_SENDING_PERIOD   (LORAWAN_SENDING_PERIOD * 2UL)
...
static void LSM6DSL_FIFO_Init(void) {
	...
	/* FIFO išvalomas prieš konfigūravimą */
	IMU_WriteReg(REG_FIFO_CTRL5, FIFO_MODE_BYPASS);

	/* Įjungiamas BDU ir auto-increment režimas */
	IMU_WriteReg(REG_CTRL3_C, 0x44);

	/* Nustatomas akselerometro 26 Hz dažnis ir ±4 g diapazonas */
	IMU_WriteReg(REG_CTRL1_XL, 0x28);

	/* Į FIFO įrašomi tik akselerometro duomenys */
	IMU_WriteReg(REG_FIFO_CTRL3, 0x01);

	/* Papildomos FIFO funkcijos nenaudojamos */
	IMU_WriteReg(REG_FIFO_CTRL4, 0x00);

	/* Įjungiamas FIFO režimas 26 Hz dažniu */
...
//------------------Nustatoma kiek duomenų yra FIFO atmintyje
...
//-----------Nuskaitomas vienas akselerometro mėginys ir išskaidomas į x,yz reikšmes
...
//-----------Gyvulio būsenos klasifikavimas pagal aktyvumo rodiklį ir Z ašies pokytį
...
//------------Sumuojamos nustatytų gyvulio būsenų trukmės sekundėmis
...
//------------20 sekundžių IMU duomenų apdorojimas
...
//----------Baitų masyvas paverčiamas į Hex
...
//------------Baterijos įkrovos skaičiavimas
static uint8_t GetBatteryPercent(void) {
	...
	battery_voltage = adc_voltage * ((30000.0f + 100000.0f) / 100000.0f);
	battery_voltage = battery_voltage * 1.024f;

	if (battery_voltage >= 4.20f)
		return 100;

	if (battery_voltage <= 3.30f)
		return 0;

	return (uint8_t) (((battery_voltage - 3.30f) / 0.90f) * 100.0f);
...
int main(void) {
	...
	uint32_t last_battery_warning_tick = 0;
	uint32_t last_led_toggle_tick = 0;
	uint8_t led_toggle_count = 0;
	uint8_t battery_warning_active = 0;
	...
	while (1) {
		...
		if (checked && rn_ok && !lora_joined
			...
			if (RN_ReadLine(line, sizeof(line), 15000) == HAL_OK) {
				if (strcmp(line, "accepted") == 0) {
					lora_joined = 1;
					last_lora_tx_tick = now - LORAWAN_SENDING_PERIOD;
				}
			}
		}
		if (lora_joined
				&& (now - last_lora_tx_tick >= LORAWAN_SENDING_PERIOD)) {
			...
			if ((now - last_gnss_location_update >= GNSS_SENDING_PERIOD)
		...
		if (battery_percent != 255 && battery_percent <= BATTERY_WARNING_LEVEL
				&& battery_warning_active == 0) {
			if (now - last_battery_warning_tick >= BATTERY_WARNING_INTERVAL_MS) {
				battery_warning_active = 1;
				led_toggle_count = 0;
				last_led_toggle_tick = now;
				last_battery_warning_tick = now;
		...
		if (battery_warning_active) {
			if (now - last_led_toggle_tick >= LED_BLINK_PERIOD_MS) {
				last_led_toggle_tick = now;
			...
			if (led_toggle_count >= LED_BLINK_TOGGLE_COUNT) {
				battery_warning_active = 0;
				led_toggle_count = 0;
